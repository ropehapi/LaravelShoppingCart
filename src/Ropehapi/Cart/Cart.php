<?php namespace Ropehapi\Cart;

use Ropehapi\Cart\Exceptions\InvalidConditionException;
use Ropehapi\Cart\Exceptions\InvalidItemException;
use Ropehapi\Cart\Helpers\Helpers;
use Ropehapi\Cart\Validators\CartItemValidator;
use Ropehapi\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package Ropehapi\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\Ropehapi\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Ropehapi\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'Ropehapi\Cart\CartCondition\'');
